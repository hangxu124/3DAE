...
if __name__ == '__main__':
    ...
    #class_weights = torch.Tensor([0.85, 0.15]).cuda()
    ...
    if not opt.no_train:
        ...
        train_logger = Logger(
            ...
            ['epoch', 'loss', 'loss1', 'loss2', 'prec1', 'prec5', 'lr'])
        train_batch_logger = Logger(
            os.path.join(opt.result_path, 'train_batch.log'),
            ['epoch', 'batch', 'iter', 'loss', 'loss1', 'loss2', 'prec1', 'prec5', 'lr'])
    ...
    if not opt.no_val:
        ...
        validation_data1 = get_validation_set(
            opt, 'all', 'top', spatial_transform, temporal_transform, target_transform)
        #validation_data2 = get_validation_set(
        #    opt, 'N', spatial_transform, temporal_transform, target_transform)
        #print ("@@@@@@@@@@@@@@@@@@@2",validation_data1.__len__())
        ...
        #    pin_memory=True,
        #    drop_last=True)
        #val_loader = [val_loader1, val_loader2]
        val_loader = val_loader1
        val_logger = Logger(
            os.path.join(opt.result_path, 'val.log'), ['epoch', 'loss', 'loss1', 'loss2', 'prec1', 'prec5'])
