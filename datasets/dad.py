...
def video_loader(video_dir_path, frame_indices, sample_duration, image_loader, camera):
    video = []
    # name = []
    #print(frame_indices)
    for i in frame_indices:
        if camera == "top":
            ...
            if (os.path.exists(image_path_D) and os.path.exists(image_path_IR)):
                video.append(image_loader(image_path_D))
                video.append(image_loader(image_path_IR))
                #name.append(image_path_IR)
            ...
            if (os.path.exists(image_path_D_f) and os.path.exists(image_path_IR)):
            ...
            print("Please select camera among 'top', 'front', 'tf'.")
    #print (len(video))
    #print (len(name))
    return video


def get_default_video_loader(camera):
    image_loader = get_default_image_loader()
    return functools.partial(video_loader, image_loader=image_loader, camera=camera)
...
def make_dataset(root_path, annotation_path, subset,motion, n_samples_for_each_video,
    ...
    for i in range(len(video_names)):
        ...
        if len(annotations) != 0:
            ...
            sample['label'] = -1
        #print (n_samples_for_each_video)
...
class Dad(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        frame_indices = frame_indices[1::2]
        ...
        clip = torch.cat(clip, 0).view((int(self.sample_duration/2), -1) + im_dim).permute(1, 0, 2, 3)
